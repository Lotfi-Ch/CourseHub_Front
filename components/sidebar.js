...
const DrawerPage = (props) => {
  ...
  React.useEffect(() => {
    ...
    if (token || loggedIn) {
      axios
        .post(`${process.env.NEXT_PUBLIC_SERVER}/api/auth/student/getOneByToken`, {
        ...
        .catch(() => {
          axios
            .post(`${process.env.NEXT_PUBLIC_SERVER}/api/auth/teacher/getOneByToken`, {
              token: token || localStorage.getItem("token"),
            })
            .then((response) => {
        ...
        <>
          <ul className="ml-4">
            <li className="mb-2 px-4 py-4 text-gray-600 flex flex-row  border-gray-300 hover:text-black   hover:bg-gray-300  hover:font-bold rounded">
              ...
              <a
                href="#"
                onClick={() => router.push(`/${props.path}/${props.id}`)}
              >
  ...
  return (
    <div className="">
      ...
      <Drawer isOpen={isOpen} toggle={toggle} position="left">
        ...
        <DrawerBody>
          <div className="mt-10 mb-4">{profileFunc()}</div>
        </DrawerBody>
        {checkToken()}
      </Drawer>
    </div>
